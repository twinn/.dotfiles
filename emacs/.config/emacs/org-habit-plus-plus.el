...
(defun org-is-habit-p (&optional pom)
  ...
  (member (org-entry-get (or pom (point)) "STYLE") `("habit" "bad-habit")))
  ;; (string= "habit" (org-entry-get (or pom (point)) "STYLE")))

(defun org-is-bad-habit-p (&optional pom)
  "Is the task at POM or point a bad-habit?"
  (string= "bad-habit" (org-entry-get (or pom (point)) "STYLE")))

(defun org-bad-habit-initial-scheduled-p (&optional pom)
  "Is the task at POM or point a bad-habit?"
  (org-entry-get (or pom (point)) "INITIAL_SCHEDULED"))
...
(defun org-habit-parse-todo (&optional pom)
  "Parse the TODO surrounding point for its habit-related data.
                  Returns a list with the following elements:
                    ...
                    7: Habit is a bad habit
                    8: Initial scheduled property
  ...
  (save-excursion
    ...
    (let* ((scheduled (org-get-scheduled-time (point)))
           ...
           (is-bad (org-is-bad-habit-p (point)))
           (initial-scheduled (org-bad-habit-initial-scheduled-p (point)))
      ...
      (let* ((maxdays (+ org-habit-preceding-days org-habit-following-days))
        ...
        (while (and (< count maxdays) (funcall search re limit t))
          ...
          (setq count (1+ count))))
      (list scheduled sr-days deadline dr-days closed-dates sr-type w-days closed-dates-weekdays is-bad initial-scheduled))))
...
(defsubst org-habit-done-dates-weekdays (habit)
  (nth 7 habit))
(defsubst org-habit-is-bad (habit)
  (nth 8 habit))
(defsubst org-habit-initial-scheduled (habit)
  (nth 9 habit))
...
(defun org-habit-is-done (done-dates start is-bad in-the-past initial-scheduled)
  (if (and is-bad)
    (org-bad-habit-is-done done-dates start in-the-past initial-scheduled)
    (and done-dates
      (org-habit--car= start (car done-dates)))))

(defun org-bad-habit-is-done (done-dates start in-the-past initial-scheduled)
  (let* ((initial-scheduled-time
           (time-to-days (org-time-string-to-time initial-scheduled)))
           (done-dates (mapcar 'car done-dates))
           (start (car start))
           )
    (message "start %s" start)
    (message "done dates %s" done-dates)
    (message "initial time %s" initial-scheduled-time)
    (if (and in-the-past (< initial-scheduled-time start))
      (or (not done-dates) (not (member start done-dates)))
      'nil)))
...
(defun org-habit-build-graph (habit starting current ending)
  ...
  (let* (;(done-dates (sort (org-habit-done-dates habit) '<))
         ...
         (s-repeat (org-habit-scheduled-repeat habit))
         (is-bad (org-habit-is-bad habit))
         (initial-scheduled (org-habit-initial-scheduled habit))
    ...
    (while (< (car start) end)
      ...
      (let* ((in-the-past-p (< (car start) now))
             (todayp (= (car start) now))
	           (donep (org-habit-is-done done-dates start is-bad in-the-past-p initial-scheduled))
...
(provide 'org-habit-plus-plus)

;;; org-habit-plus-plus.el ends here
